                        ...
                        <tbody>
                            @forelse ($suratList as $index => $surat)
                                <tr>
                                    ...
                                    <td>
                                        @if ($surat->file_surat_pengajuan)
                                            @if ($surat->file_pengajuan_exists)
                                                ...
                                                <small
                                                    class="text-muted">{{ $surat->tanggal_upload_pengajuan ? $surat->tanggal_upload_pengajuan->format('d M Y H:i') : '-' }}</small>
                                            @else
                                    ...
                                    <td>
                                        @if ($surat->status_balasan)
                                            @if ($surat->status_balasan == 'diterima')
                                                <span class="badge bg-success">
                                                    <i class="fas fa-check-circle me-1"></i>Diterima
                                                </span>
                                            @elseif($surat->status_balasan == 'ditolak')
                                                <span class="badge bg-danger">
                                            ...
                                            <br>
                                            <small
                                                class="text-muted">{{ $surat->tanggal_upload_balasan ? $surat->tanggal_upload_balasan->format('d M Y H:i') : '-' }}</small>
                                        @else
                                    ...
                                    <td>
                                        @if ($surat->file_surat_balasan)
                                            @if ($surat->file_balasan_exists)
                                                <a href="/admin/surat-dudi/{{ $surat->id }}/download?type=balasan"
                                    ...
                                    <td>
                                        @if ($surat->catatan_dudi)
                                            <button class="btn btn-sm btn-info" data-bs-toggle="modal"
                                                data-bs-target="#catatanModal{{ $surat->id }}">
                                                <i class="fas fa-eye me-1"></i>Lihat
                                            </button>

                                            <!-- Modal Catatan -->
                                            <div class="modal fade" id="catatanModal{{ $surat->id }}" tabindex="-1">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            <p><strong>DUDI:</strong> {{ $surat->dudi->nama_dudi }}</p>
                                                            <hr>
                                                            <p>{{ $surat->catatan_dudi }}</p>
                                                        </div>
